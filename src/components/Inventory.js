...
export default class Inventory extends Component {
  ...
  authHandler = (err, authData) => {
    ...
    storeRef.once('value', (snapshot) => {
      ...
      // claims it as our on if there's no owner already
      if(!data.owner) {
        storeRef.set({
  ...
  render() {
    ...
    // Is the user the owner of current store?
    if(this.state.uid !== this.state.owner) {
      return (
        <div>
          <p>
            Sorry, you're now the owner of this store! :(
            {logout}
          </p>
        </div>
      )
